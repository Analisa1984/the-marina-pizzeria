...
class Booking(models.Model):
    ...
    def __str__(self):
        return (
            f"Booking for {self.User_id.first_name} {self.User_id.last_name} "
            f"on {self.booking_date} at {self.booking_time} "
            f"for {self.party_number} people."
        )
...
class Table(models.Model):
    ...
    table_number = models.PositiveIntegerField(
        validators=[MinValueValidator(20), MaxValueValidator(30)], blank=False
    )
